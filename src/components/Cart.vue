<template>
  ...
  >
    ...
    <div
      class="flex-1 space-y-3 overflow-y-auto px-5 py-4 text-slate-800 scrollbar-hide"
    >
      <p
      ...
      >
        ...
        <div class="flex flex-1 flex-col">
          <p class="text-sm font-bold text-slate-900 line-clamp-1">
            {{ entry.name }}
          </p>
          ...
          <div class="mt-2 flex items-center justify-between">
            <div
              class="flex items-center gap-3 rounded-full bg-white/40 px-2 py-0.5 shadow-inner backdrop-blur-sm"
            >
              <button
              ...
              <span class="w-4 text-center text-xs font-bold text-slate-800">{{
                entry.qty
              }}</span>
              <button
    ...
    <div
      class="border-t border-white/20 bg-white/30 px-5 pt-3 pb-5 backdrop-blur-xl"
    >
      <div class="mb-3 flex items-center justify-between px-1">
        <span class="text-sm font-medium text-slate-700">Total</span>
        <span
          class="text-xl font-black text-slate-900 shadow-sm"
...
async function handlePlaceOrder() {
  ...
  try {
    ...
    addAlert(
      'Order placed successfully! Please pay at the counter.',
      'success'
    );
  } catch (e) {
